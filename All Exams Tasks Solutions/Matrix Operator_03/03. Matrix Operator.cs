    ...
    class Program
    {
        static void Main()
        {
            ...
            while (true)
            {
                ...
                if (command == "remove") matrix = RemoveElements(matrix, data);
                else if (command == "swap") matrix = SwapRows(matrix, data);
                else if (command == "insert") matrix = InsertElementInRow(matrix, data);
            }
            PrintMatrix(matrix);
        }
        ...
        private static List<List<int>> SwapRows(List<List<int>> matrix, string[] data)
        {
            ...
            bool isValidIndex = row1 >= 0 && row1 < matrix.Count &&
                                row2 >= 0 && row2 < matrix.Count;
            if (isValidIndex)
            {
        ...
        private static List<List<int>> RemoveElements(List<List<int>> matrix, string[] data)
        {
            ...
            if (rowCol == "row" && isValidRowIndex)
            {
                ...
                if (type == "positive") elements.RemoveAll(x => x >= 0);    // NB! =
                ...
                matrix[index] = elements;       // empty rows are not deleted
            }
            else if (rowCol == "col")
            {
                for (int row = 0; row < matrix.Count; row++)    // valid row index
                {
                    bool isValidColIndex = index >= 0 && index < matrix[row].Count;
                    if (isValidColIndex)
                    {
                        List<int> elements = matrix[row];
                        if (type == "positive" && elements[index] >= 0) elements.RemoveAt(index);   // NB! =
